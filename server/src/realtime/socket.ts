  ...
  io.on("connection", (socket) => {
    ...
    socket.on("send_message", async ({ roomId, content }, ack) => {
      try {
        ...
        // Frontend expects the message to belong to the room identifier they know (e.g. slug)
        // ensureUserInRoom/createMessage resolved this to a UUID, but we send back the slug (or whatever ID was used)
        io.to(roomId).emit("receive_message", { ...result.message, roomId });
